...
def _return_prefix(
    _: typing.Any, __: typing.Any, *, prefixes: typing.Iterable[str]
) -> typing.Sequence[str]:
    return prefixes
...
class Bot(hikari.BotApp):
    ...
    @staticmethod
    def print_banner(
        banner: typing.Optional[str], allow_color: bool, force_color: bool
    ) -> None:
    ...
    def add_check(
        self,
        func: typing.Callable[[context_.Context], typing.Coroutine[None, None, bool]],
    ) -> None:
        """
    ...
    def add_plugin(self, plugin: plugins.Plugin) -> None:
        ...
        for event_type, listeners in plugin.listeners.items():
            for listener in listeners:
                ...
                _LOGGER.debug(
                    "new listener registered: %s (%s)",
                    callback.__name__,
                    listener.event_type.__name__,
                )
    ...
    def load_extension(self, extension: str) -> None:
        ...
        if not hasattr(module, "load"):
            raise errors.ExtensionMissingLoad(
                text=f"{extension} is missing a load function"
            )
        else:
            module.load(self)
    ...
    def unload_extension(self, extension: str) -> None:
        ...
        if not hasattr(module, "unload"):
            raise errors.ExtensionMissingUnload(
                text=f"{extension} is missing an unload function"
            )
        else:
            module.unload(self)
    ...
    async def send_help(
        ...
        obj: typing.Union[commands.Command, plugins.Plugin] = None,
    ) -> None:
        """
    ...
    async def _evaluate_checks(
        self, command: commands.Command, context: context_.Context
    ) -> bool:
            ...
            try:
                if not await check(context):
                    failed_checks.append(
                        errors.CheckFailure(
                            f"Check {check.__name__} failed for command {command.name}"
                        )
                    )
    ...
    async def _dispatch_command_error_event_from_exception(
        ...
        if command is not None:
            ...
            if not handled and self.get_listeners(
                events.CommandErrorEvent, polymorphic=True
            ):
    ...
    def resolve_args_for_command(
        ...
        if (
            remainder
            and command.arg_details.kwarg_name is None
            and not command._allow_extra_arguments
        ):
            raise errors.TooManyArguments(command)
        if (
            len(positional_args) + bool(remainder)
        ) < command.arg_details.minimum_arguments:
    ...
    async def process_commands_for_event(
        self, event: hikari.MessageCreateEvent
    ) -> None:
        """
        ...
        await self.dispatch(
            events.CommandInvocationEvent(app=self, command=command, context=context)
        )
        if (before_invoke := command._before_invoke) is not None:
            await before_invoke(context)

        try:
            positional_args, keyword_arg = self.resolve_args_for_command(
                command, final_args
            )
            ...
            await self._dispatch_command_error_event_from_exception(
                ex, event.message, context, command
            )
            return
        ...
        try:
            ...
            await self._dispatch_command_error_event_from_exception(
                new_ex, event.message, context, command
            )
            return
        ...
        await self.dispatch(
            events.CommandCompletionEvent(app=self, command=command, context=context)
        )
