...
class HelpCommand:
    ...
    async def send_paginated_help(
        text: typing.Sequence[str], context: context_.Context
    ) -> None:
        """
    ...
    async def resolve_help_obj(
        self, context: context_.Context, obj: typing.List[str]
    ) -> None:
        """
    ...
    async def send_plugin_help(
        self, context: context_.Context, plugin: plugins.Plugin
    ) -> None:
        """
        ...
        help_text = [
            ...
            ", ".join(
                f"`{c.name}`"
                for c in sorted(plugin._commands.values(), key=lambda c: c.name)
            )
    ...
    async def send_command_help(
        self, context: context_.Context, command: commands.Command
    ) -> None:
        """
        Method called when the help command is run with an argument that
    ...
    async def send_group_help(
        self, context: context_.Context, group: commands.Group
    ) -> None:
        """
        Method called when the help command is run with an argument that
