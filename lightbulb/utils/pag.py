        ...
        *,
        ...
        if max_lines is not None:
            if max_lines < min_total_lines:
                raise ValueError(
                    f"This configuration requires at least {min_total_lines} lines per page!"
                )
        ...
        if max_chars < min_total_len:
            raise ValueError(
                f"This configuration requires at least {min_total_len} characters per page."
            )
        ...
        self._max_content_lines = (
            max_lines - extra_lines if max_lines is not None else float("inf")
        )
...
class EmbedPaginator(Paginator[hikari.Embed]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            suffix=suffix,
            line_separator=line_separator,
            page_factory=lambda i, s: hikari.Embed(description=s).set_footer(
                text=f"Page {i}"
            ),
        )
    ...
    def set_embed_factory(
        self, func: typing.Callable[[int, str], hikari.Embed]
    ) -> None:
        """
