...
def _get_missing_perms(
    required_permissions: hikari.Permissions, roles: typing.Sequence[hikari.Role]
) -> hikari.Permissions:
    missing_perms = hikari.Permissions.NONE
    user_permissions = set([role.permissions for role in roles])

    for user_permission in user_permissions:
        if hikari.Permissions.ADMINISTRATOR in user_permission:
            return missing_perms
        if user_permission not in required_permissions:
            missing_perms |= user_permission

    return missing_perms


async def _has_guild_permissions(
    ctx: context.Context, *, permissions: hikari.Permissions
):
    ...
    if missing_perms:
        raise errors.MissingRequiredPermission(
            f"You don't have these perms", permissions
        )
    return True


async def _bot_has_guild_permissions(
    ctx: context.Context, *, permissions: hikari.Permissions
):
    if not (ctx.bot.intents & hikari.Intents.GUILDS) == hikari.Intents.GUILDS:
        raise hikari.MissingIntentError(hikari.Intents.GUILDS)
    ...
    if missing_perms:
        raise errors.BotMissingRequiredPermission(
            "I am missing one or more permissions required in order to run this command",
            missing_perms,
        )
    return True
...
async def _has_permissions(
    ...
    allowed_perms = functools.reduce(
        operator.or_,
        (
            override.allow if override.id in ctx.member.role_ids else perm_none
...
async def _bot_has_permissions(
    ctx: context.Context, *, permissions: hikari.Permissions
) -> bool:
    if not (ctx.bot.intents & hikari.Intents.GUILDS) == hikari.Intents.GUILDS:
        raise hikari.MissingIntentError(hikari.Intents.GUILDS)
    ...
    allowed_perms = functools.reduce(
        operator.or_,
        (
            override.allow if override.id in bot_member.role_ids else perm_none
            for override in perm_over
        ),
    )
    ...
    *role_ids: snowflakes.SnowflakeishOr[hikari.PartialRole],
    ...
    def decorate(command: T_inv) -> T_inv:
        ...
        check_func = functools.partial(
            _role_check,
            roles=[int(role1), *[int(r) for r in role_ids]],
            func=all if mode == "all" else any,
        )
...
def has_guild_permissions(perm1: hikari.Permissions, *permissions: hikari.Permissions):
    ...
    def decorate(command: T_inv) -> T_inv:
        ...
        command.add_check(
            functools.partial(_has_guild_permissions, permissions=total_perms)
        )
        return command
...
def bot_has_guild_permissions(
    ...
    def decorate(command: T_inv) -> T_inv:
        ...
        command.add_check(
            functools.partial(_bot_has_guild_permissions, permissions=total_perms)
        )
        return command
...
def bot_has_permissions(perm1: hikari.Permissions, *permissions: hikari.Permissions):
    ...
    def decorate(command: T_inv) -> T_inv:
        ...
        command.add_check(
            functools.partial(_bot_has_permissions, permissions=total_perms)
        )
        return command
...
def check(
    check_func: typing.Callable[
        [context.Context], typing.Coroutine[typing.Any, typing.Any, bool]
    ]
):
    """
