...
async def _get_or_fetch_guild_channel_from_id(
    arg: WrappedArg, channel_id: hikari.Snowflake
) -> hikari.GuildChannel:
...
def _raise_if_none(obj: typing.Optional[T]) -> T:
    if obj is None:
...
async def user_converter(arg: WrappedArg) -> hikari.User:
    ...
    try:
        ...
        user = utils.find(
            users.values(),
            lambda u: u.username == arg.data
            or f"{u.username}#{u.discriminator}" == arg.data,
        )
    else:
        user = arg.context.bot.cache.get_user(user_id)
        if user is None:
            user = await arg.context.bot.rest.fetch_user(user_id)
    return _raise_if_none(user)
...
async def member_converter(arg: WrappedArg) -> hikari.Member:
    ...
    try:
        ...
        member = utils.find(
            members.values(),
            lambda m: str(m.username).lower() == str(arg.data).lower()
            or str(m.nickname).lower() == str(arg.data).lower()
...
def voice_channel_converter(
    arg: WrappedArg,
) -> typing.Coroutine[typing.Any, typing.Any, hikari.GuildVoiceChannel]:
    warnings.warn(
